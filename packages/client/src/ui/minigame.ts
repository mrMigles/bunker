  ...
  private show(mg: MiniDef & { action: string }) {
    ...
    const body = h(
      ...
      h(
        "div.row",
        { style: { marginTop: "8px" } },
        h("span.dim", { style: { fontSize: "12px" } }, "Руками дело идёт в 2–4 раза быстрее, чем само. «Пусть делает сам» — персонаж доделает в обычном темпе (пока окно открыто, само оно почти стоит)."),
        h("div.grow"),
